function PlayScreen(element) {
  ...
  this._element.ownerDocument.addEventListener(
    "keydown",
...
Object.defineProperties(PlayScreen.prototype, {
  update: {
    value: function update(elapsedTime) {
      ...
      if (this._playerBurger && this._playerBurger.length > 0) {
        background = BurgerVisualizer.convertBurgerToCssBackground(
          this._playerBurger,
          200
        );
      }
      ...
      this._playerScore.innerText =
        "$" + this._gameEngine._currentScore.toFixed(2);
      this._playerLevel.innerText = this._gameEngine.levelName;
      this._playerLives.innerText = this._gameEngine._currentLives;
    }
  },
  ...
  addCustomer: {
    value: function _addCustomer(customer) {
      customer.desiredX =
        this._firstCustomerPosition +
        this._offsetPerCustomer * this._customers.length;
  ...
  removeCustomer: {
    value: function _removeCustomer(customer) {
      ...
      for (var i = 0; i < this._customers.length; i++) {
        this._customers[i].desiredX =
          this._firstCustomerPosition + this._offsetPerCustomer * i;
      }
    }
  },
  addIngredient: {
