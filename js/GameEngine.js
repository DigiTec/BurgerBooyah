...
(function () {
    var _scoreToLevelConverter = [ 0,24.75,51.15,79.2,123.75,170.775,233.475,296.175,380.655,476.685,584.485 ];
    // var _scoreToLevelConverter = [ 0, 42, 90, 146, 210, 284, 362, 446, 535, 632, 740 ];
    var _levelToTileConverter = [ 
        "Tutorial", "Just Burgers", "In Comes the Big Cheese!", "It's On the House!", "Make Mine a Double!!", 
        "You're the Mac Daddy!", "Training Wheels Off", "Your Burger, Your Way!", "Shoot for a High Score!!!"
    ];
    ...
    Object.defineProperties(GameEngine.prototype, {
        update: {
            value: function update(elapsedTime) {
                ...
                if (this._spawnElapsed >= this.currentSpawnDuration && this._screen.customerCount < this._maxCustomers) {
                    var burger;
                    if (this._currentLevel <= 5 || Math.random() < 0.8) {
                        var knownBurgerMax = Math.min(BurgerBuilder.CUSTOM_BURGERS.length, this._currentLevel);
                        burger = BurgerBuilder.knownBurger(parseInt(Math.random()*knownBurgerMax));
                    }
                    else {
                        var maxIngredients = Math.min(this._currentLevel, 10);
                        burger = BurgerBuilder.randomIngredientBurger(BurgerBuilder.MIN_INGREDIENTS, maxIngredients);
                    }
        ...
        scoreCustomer: {
            value: function scoreCustomer(customer) {
                this._currentScore += (3 + (customer._burger.length * 0.25)) * customer._happiness;
        ...
        currentSpawnDuration: {
            get: function get_currentSpawnDuration() {
                ...
                return Math.max(3000, 5000 - ((this._currentLevel - BurgerBuilder.CUSTOM_BURGERS.length) * 200));
            },
        },
