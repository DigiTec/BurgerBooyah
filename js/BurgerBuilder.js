  ...
  Object.defineProperties(BurgerBuilder, {
    ...
    randomIngredientBurger: {
      value: function randomIngredientBurger(minIngredients, maxIngredients) {
        ...
        var trueMax = Math.max(
          maxIngredients || minIngredients,
          minIngredients
        );
        var randMax = Math.round(
          Math.random() * (trueMax - minIngredients) + minIngredients
        );
        ...
        while (burger.length < randMax) {
          ...
          var newIngredient =
            currentIngredientList[
              parseInt(Math.random() * currentIngredientList.length)
            ];
          ...
          if (burger.length == randMax - 2) {
            if (
              BurgerBuilder.canStack(
                BurgerBuilder.INGREDIENT_BUNTOP,
                newIngredient
              )
            ) {
  ...
  convertBurgerToCssBackground: {
    value: function convertBurgerToCssBackground(burger, targetHeight) {
      ...
      for (var i = 0; i < burger.length; i++) {
        images.push(
          _ingredientToImageMap[burger[i]] +
            "0px " +
            currentOffset +
            "px no-repeat"
        );
