function CustomerEntity(burger, screen) {
  ...
  this._sprite.style.background =
    "url('images/character_bodies_boy.png') " + offset + "px 0px no-repeat";
...
Object.defineProperties(CustomerEntity.prototype, {
  ...
  onPointerDown: {
    value: function _onPointerDown() {
      if (!this._removeOnArrival) {
        if (
          this._orderContainer &&
          this._screen.isCustomerSatisfiedWithOrder(this)
        ) {
  ...
  showOrder: {
    value: function _showOrder() {
      ...
      var bg = BurgerVisualizer.convertBurgerToCssBackground(
        this._burger,
        burgerHeight
      );
