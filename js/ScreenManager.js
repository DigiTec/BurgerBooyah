...
function SwipeAnimationOptions(timeToBlend, swipeDirection, blendOpacity) {
    ...
    this.swipeDirection = swipeDirection || SwipeAnimationOptions.SWIPEDIRECTION_RIGHT;
...
var SwipeTransition = (function () {
    var transitionFunc = function swipeTransition(from, to, finalizer) {
        this.startTime = new Date();
        this.from = from; this.from.style.zIndex = 0;
        this.to = to; this.to.style.zIndex = 1;
        this.finalizer = finalizer;
        this.startPositionX = 0;
        this.startPositionY = 0;
        switch (this.swipeDirection) {
            case SwipeAnimationOptions.SWIPEDIRECTION_TOP: this.startPositionY = -from.offsetHeight; break;
            case SwipeAnimationOptions.SWIPEDIRECTION_LEFT: this.startPositionX = -from.offsetWidth; break;
            case SwipeAnimationOptions.SWIPEDIRECTION_RIGHT: this.startPositionX = from.offsetWidth; break;
            case SwipeAnimationOptions.SWIPEDIRECTION_BOTTOM: this.startPositionY = from.offsetHeight; break;
        }
        ...
        setupScreens: {
            value: function setupScreens(screenOwner, classScreen, classShow) {
                ...
                while (screenIterator !== null) {
                    if (screenIterator.className.split(' ').indexOf(this.classScreen) >= 0) {
                        _screens[screenIterator.id] = screenIterator;
                        screenIterator.className = removeClass(screenIterator.className, classShow);
                    }
        ...
        transitionToScreen: {
            value: function transitionToScreen(screenName, transition) {
                var finalizeTransition = (function finalizeTransition(from, to) {
                    if (_currentScreen) {
                        _currentScreen.className = removeClass(_currentScreen.className, this.classShow);
                        _currentScreen = null;
                    }
